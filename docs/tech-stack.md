...
1. **放在自己模块的测试目录下**：`app/src/test/java/sg/edu/nus/serms/<模块>/`，和 `main` 里的包结构一一对应。
2. **三层都要有**：
   - **单元测试**：领域规则、策略类、状态机。不连数据库，毫秒级完成（JUnit 5 + Mockito）。
   - **集成测试**：service + repository + 真实 PostgreSQL（Spring Boot Test + Testcontainers），覆盖事务、约束和并发。
   - **Web / 权限测试**：controller 的正常请求、校验失败、无权限访问、CSRF（MockMvc + spring-security-test）。
3. **覆盖用例的正常流程和所有主要异常流程**：按下表逐条写，每条异常流程至少对应一个测试方法。
4. **能自己跑起来**：本机只要装了 Docker，执行 `./mvnw verify` 就能跑完全部测试。不需要手工建库、设置环境变量或运行 PowerShell 脚本；CI 上执行的也是同一条命令。
5. **不依赖其他测试**：每个测试自己准备数据（用随机 UUID 或唯一编号），不依赖执行顺序，不清空别人会用到的表，不假设表里 “只有我的数据”。
6. **测的是行为而不是实现**：断言 “业务结果 / 数据库状态 / HTTP 响应”，不断言内部调用了哪个私有方法。
7. **和功能代码在同一个 PR 里提交**。新增行为没有对应测试的 PR 不得合并；覆盖率低于 70%（service + domain）时 CI 直接失败。

### 5.2 每位成员的测试台范围

| 成员 | 负责用例 | 测试台至少覆盖 |
| --- | --- | --- |
| Shi Wenqi | 搜索并预约设备 | 按条件搜索；指定时段可用性；预约成功；时间冲突（含两个请求并发抢同一时段，只能成功一个）；设备维修中 / 报废不可预约；超出时长或配额被规则拒绝；借用人取消预约；领用后不可取消 |
| Zhang Hanming | 审核和批准预约 | 无需审批的设备直接确认；批准；拒绝并填写意见（拒绝后时段被释放）；不能审批自己的预约；非审批人被拒绝访问；审批记录写入后不可修改 |
| Zhou Fanhao | 设备领用和归还 | 正常领用；非法领用（未批准、不在领用时间窗、设备不匹配）；正常归还；逾期归还；损坏归还后自动转入维修；同一设备并发领用只能成功一次 |
| Liu Tongyao | 故障报告和维修处理 | 报告故障后设备立即不可预约；分配技术员；维修状态六步流转及非法流转被拒绝；修复后设备恢复可用；无法修复后设备报废；维修历史完整保留 |
| Wang Yuanmeng | 逾期检测和通知 | 即将到期提醒；已经逾期提醒；通知投递成功；投递失败后重试，达到上限后标记失败；重复提醒只生成一次；设备已归还时取消未发送的提醒；收件人只能看到自己的通知 |

公共测试基础设施由 **Zhang Hanming**（测试策略负责人）维护：Testcontainers 的 PostgreSQL 基类、测试数据构造工具、ArchUnit 分层规则。其他成员直接复用，不要各自再写一套数据库启动代码。

### 5.3 CI 如何使用这些测试台

- 每个 PR 自动运行 `./mvnw verify`：全部模块的单元、集成、Web 测试都会运行，并做 JaCoCo 覆盖率检查和 ArchUnit 分层检查。
- 任何一个测试失败，PR 都不能合并。修改自己模块时如果把别人的测试跑挂了，说明破坏了别人的功能，要先修好再合并。
- 测试报告（Surefire）和覆盖率报告（JaCoCo）作为 CI 产物上传，直接作为 DoD 要求的 “测试证据”。
- 不允许为了让 CI 变绿而跳过、删除或注释掉测试（`@Disabled` 必须在 PR 描述里说明原因，并由另一位成员同意）。
