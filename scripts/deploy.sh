...
# 3) 构建并滚动到新容器
HOST_PORT="$(grep -E '^HOST_PORT=' .env 2>/dev/null | cut -d= -f2 | tr -d ' ' || true)"; HOST_PORT="${HOST_PORT:-8789}"
HEALTH_PATH="$(grep -E '^HEALTH_PATH=' .env 2>/dev/null | cut -d= -f2 | tr -d ' ' || true)"; HEALTH_PATH="${HEALTH_PATH:-/api/health}"
